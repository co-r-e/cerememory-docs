...
<Callout type="info">
The audit log is separate from operational logging (`[log]`) and from Prometheus metrics. It records *operation metadata* (which records, which store, what kind of operation) — never the memory content itself — and is intended to be retained alongside backups for forensic review.
</Callout>
