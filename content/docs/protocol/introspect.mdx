...
The following query parameters are defined in the CMP specification. In the current HTTP transport implementation, these default to `false` and are not configurable via query string -- the full record (including associations) is always returned.
...
<Callout type="info">
These parameters are available when using the CMP protocol directly (e.g., via gRPC or MCP). The HTTP REST endpoint returns the standard record with associations included.
</Callout>
