...
- **Rust 1.95+** -- Install via [rustup](https://rustup.rs/). The workspace pins this MSRV via `rust-toolchain.toml` and CI enforces it.
...
<Callout type="info">
The workspace contains 19 crates plus three LLM adapter crates. Use `cargo build -p cerememory-core` to build a specific crate during focused development.
</Callout>
