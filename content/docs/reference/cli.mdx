...
<Callout type="info">
The MCP surface is intentionally LLM-friendly and does not expose every HTTP/gRPC method one-to-one. `export` requires an explicit `output_path`, **always includes the raw journal**, and archive import is intentionally unavailable over MCP. The output path is hardened against overwrite, symlink, missing-parent, and parent-traversal writes. For full protocol access, use the REST API or gRPC transport.
</Callout>
