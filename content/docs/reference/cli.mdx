...
| `--emotion` | string | None | Emotion label (e.g., `joy`, `trust`, `fear`) |
...
```bash
cerememory recall-raw --session-id sess-001 --query "editor preference"
```
</Terminal>
...
| `export` | Export a CMA archive to an explicit output path |

<Callout type="info">
The MCP surface is intentionally LLM-friendly and does not expose every HTTP/gRPC method one-to-one. For full protocol access, use the REST API or gRPC transport.
</Callout>
